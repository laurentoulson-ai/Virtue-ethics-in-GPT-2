"""
main_virtue_ethics.py found N87 to be highly specialised but on running virtue_circuit.py N87 was not found to be one of the top active neurons when using
my new refined data set of statements that were supposed to be highly active for N87 ...
So I am re-running main_virtue_ethics.py with the adjustment using my refined dataset to see how this impacts the results
Because of the difference in data set sizes I've adjusted normalisation from original programme.
"""
...
def load_statements():
    ...
    virtue_ethics = []
    with open('data/virtue_top20.txt', 'r') as f:
        for line in f:
...
def calculate_specialization(virtue_acts, neutral_acts, virtue_count, neutral_count):
    """Normalize activation rates by dataset size"""
    specialized_neurons = []
    
    # Normalize by dataset size
    virtue_total = max(1, len(virtue_acts) / virtue_count)
    neutral_total = max(1, len(neutral_acts) / neutral_count)
    ...
    for neuron in all_neurons:
        ...
        # Relaxed criteria for specialization
        meets_criteria = (
...
def main():
    ...
    specialized_neurons = calculate_specialization(
        virtue_activations, neutral_activations, len(virtue_ethics), len(neutral)
    )
